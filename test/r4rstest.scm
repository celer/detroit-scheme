...
(SECTION 4 2 4)
; XXX: this test fails but it doesn't actualy fail could be bad vector implementation
...
(test 10 'define (foo))
(define old-+ +)
; XXX: yeah this unrolls incorrectly
;(begin (begin (begin)
...
;			    (define bar (foo #f))
;			    (foo #f)))
; XXX: i guess these are most likely due to define in letrec 
...
(test #t equal? 2 2)
; XXX: another unespected usage of equal? not matching 
...
; XXX: i don't understand how these tests work 
;(test '() append)
;(test '(a b c . d) append '(a b) '(c . d))
;(test 'a append '() 'a)
...
; XXX: this whole set of test is against stdlib
...
; XXX: probably native java symbol string interning
...
; XXX: i think these are actually in an srfi not stdlib 
;(test 1 expt 0 0)
;(test 0 expt 0 1)
;(test 0 expt 0 256)
;;(test 0 expt 0 -255)
...
(test #f odd? 2)
(test #f odd? -4)
; XXX: behavior should be odd here
;(test #t odd? -1)
(test #f even? 3)
(test #t even? 2)
(test #t even? -4)
...
(test '(1 2 3) map * '(1 2 3))
(test '(-1 -2 -3) map - '(1 2 3))
; XXX: broken vector building with for-each
...
; XXX: does not serialize as test expects
;(test #t call-with-output-file
;      "./test/tmp1"
;      (lambda (test-file)
;	(write-char #\; test-file)
...
;	(write load-test-obj test-file)
;	(output-port? test-file)))
;(check-test-file "./test/tmp1")

;(define test-file (open-output-file "./test/tmp2"))
;(write-char #\; test-file)
;(display #\; test-file)
;(display ";" test-file)
;(write write-test-obj test-file)
...
(report-errs)
; XXX: i wonder why this comes at the end of the test
; 	seems to attempt to test only if inexacts exists wtf?
;(let ((have-inexacts?
