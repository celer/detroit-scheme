...
(define (test:jdbc:setup)
  ...
  ; create the database
  (test:jdbc:query "create table test (id INTEGER PRIMARY KEY, data TEXT, num double, timeEnter DATE);")
  ; create a datetime trigger
  (test:jdbc:query "CREATE TRIGGER insert_test_timeEnter AFTER  INSERT ON test BEGIN UPDATE test SET timeEnter = DATETIME('NOW')  WHERE rowid = new.rowid; END;"))
